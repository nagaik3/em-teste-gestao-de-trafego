...
import { gold, goldDim, card, border, surface, textSecondary, pill } from "../styles/theme"
  ...
  return (
    <div>
      ...
      <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: 20, marginBottom: 20 }}>
        ...
        {nicho && <>
          <p style={{ color: textSecondary, fontSize: 11, textTransform: "uppercase", letterSpacing: 1, margin: "16px 0 10px" }}>Regiao</p>
      ...
      {nicho && regiao && (
        <div>
          <p style={{ color: textSecondary, fontSize: 11, textTransform: "uppercase", letterSpacing: 1, marginBottom: 12 }}>
          ...
          {tasks.map(t => {
            ...
            return <button key={t.id} onClick={() => setSelectedId(t.id)} style={{
              ...
              <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                {t.copywriter && <span style={{ color: textSecondary, fontSize: 12 }}>{t.copywriter}</span>}
      ...
      {selectedId && detail && !showConfirm && (
        <div style={{ position: "fixed", top: 0, right: 0, width: "min(460px, 100vw)", height: "100vh", background: surface, borderLeft: `1px solid ${border}`, overflowY: "auto", zIndex: 50, boxShadow: "0 8px 24px rgba(0,0,0,0.6)" }}>
          <div style={{ position: "sticky", top: 0, background: surface, borderBottom: `1px solid ${border}`, padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button onClick={() => setSelectedId(null)} style={{ background: "none", border: "none", color: textSecondary, fontSize: 18, cursor: "pointer" }}>x</button>
          </div>
          <div style={{ padding: 20 }}>
            ...
            {detail.description && <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 10, padding: 12, marginBottom: 16, maxHeight: 160, overflowY: "auto" }}><p style={{ color: textSecondary, fontSize: 13, whiteSpace: "pre-wrap" }}>{detail.description}</p></div>}
            {detail.checklists.map((cl, i) => (
              <div key={i} style={{ background: card, border: `1px solid ${border}`, borderRadius: 10, padding: 12, marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ color: textSecondary, fontSize: 12 }}>{cl.name}</span>
                ...
                {cl.items.map((item, j) => <div key={j} style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 2 }}>
                  ...
                  <span style={{ fontSize: 12, color: item.resolved ? textSecondary : "#555a6e", textDecoration: item.resolved ? "line-through" : "none" }}>{item.name}</span>
                </div>)}
              </div>
            ))}
      ...
      {showConfirm && detail && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 60 }}>
          <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 12, padding: 24, width: 400, maxWidth: "90vw" }}>
            ...
            <p style={{ color: textSecondary, fontSize: 13, marginBottom: 16 }}><strong style={{ color: "#eceef2" }}>{gestor.nome}</strong> sera atribuido como gestor. A tarefa sera movida para <strong style={{ color: "#eceef2" }}>Em Teste</strong> e o copywriter sera notificado.</p>
            <div style={{ background: "#181b24", border: `1px solid ${border}`, borderRadius: 8, padding: 12, marginBottom: 20 }}><p style={{ fontFamily: "monospace", fontSize: 12, color: "#eceef2", wordBreak: "break-all" }}>{detail.name}</p></div>
            <div style={{ display: "flex", gap: 12 }}>
              <button onClick={() => setShowConfirm(false)} disabled={claim.isPending} style={{ flex: 1, padding: 10, background: "#181b24", border: `1px solid ${border}`, borderRadius: 8, color: textSecondary, fontSize: 13, cursor: "pointer" }}>Cancelar</button>
